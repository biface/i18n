...
class TestIsValidLanguageTag:
    ...
    def test_is_valid_language_tag(slef, tag, result):
...
class TestNormalizeLanguageTag:
    ...
    def test_normalize_language_tag(self, tag, result):
        assert normalize_language_tag(tag) == result

    def test_normalize_language_tag_invalid(self):
        with pytest.raises(ValueError):
            normalize_language_tag("ja.Latn/hepburn@heploc")


class TestValidateAndNormalizeLanguageTags:
    @pytest.mark.parametrize(
        "tags, expected",
        [
            (
    ...
    def test_validate_and_normalize_language_tags_valid(self, tags, expected):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ["en-US", "invalid-tag", "de"],
                "invalid-tag",
            ),  # 'invalid-tag' est invalide
        ],
    )
    def test_validate_and_normalize_language_tags_invalid(self, tags, invalid_tag):
...
class TestGetAllLanguages:
    ...
    def test_get_all_languages(self, hierarchy, expected_output):
