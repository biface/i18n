...
class TestRepositoryProperties:
    ...
    @pytest.mark.parametrize("path, error, error_msg", [
        (["/false/path"], TypeError, "Repository must be a string, not <class 'list'"),
        ("path/../test-function", FileNotFoundError, "Repository path/../test-function does not exist"),
        ("/tmp/absolute_non_existent_path/", FileNotFoundError, "Repository /tmp/absolute_non_existent_path/ does not exist"),
    ])
    def test_repository_set_repository_failed(self, test_repository, path, error, error_msg):
    ...
    @pytest.mark.parametrize(
        "hierarchy, expected", [
            ({"it": ["it-IT"]}, [("it", ["it-IT"])]),
            ({"fr": ["fr-FR"], "en": ["en-US", "en-GB"]}, [("fr", ["fr-FR"]), ("en", ["en-US", "en-GB"])]), ]
    )
    ...
    @pytest.mark.parametrize(
        "hierarchy, error_msg", [
            (["it", ["it-IT"]],
             "hierarchy must be a dictionary, not <class 'list'>"),
            ({"fr": [1], "en": ["en-US", "en-GB"]},
    ...
    def test_repository_set_hierarchy_failed(self, test_repository, hierarchy, error_msg):
...
class TestRepositoryMethods:
    ...
    @pytest.mark.parametrize("fallback, languages, expected", [
        ("fr", "fr-BE", ["fr-FR", "fr-BE"]),
        ("it", ["it-IT", "it-CH"], ["it-IT", "it-CH"]),
        ("se", ["se-FI", "se-NO"], ["se-FI", "se-NO"]),
        ("se", "se-SE", ["se-FI", "se-NO", "se-SE"]),
    ])
    def test_repository_add_hierarchy(self, test_repository, fallback, languages, expected):
        test_repository.add_hierarchy(fallback, languages)
        assert test_repository[["languages", "hierarchy", fallback]] == expected

    @pytest.mark.parametrize("fallback, languages, error, error_msg", [
        (["rf"], "rf-BE", TypeError, "Fallback must be a string, not <class 'list'>"),
        ("rf", "rf-BE", ValueError, "Fallback language must be a compliant IETF Tag, not rf"),
        ("fr", 1, TypeError, "Languages must be a string or a list of strings, not <class 'int'>"),
        ("fr", [1, "fr-MO"], TypeError, "Language hierarchy must be a list of string, not <class 'int'>"),
        ("fr", "fr-EB", ValueError, "Language in hierarchy value must be a compliant IETF Tag, not fr-EB"),
        ("fr", "fr-BE", ValueError, "Language fr-BE already exists for fallback fr")
    ])
    def test_repository_add_hierarchy_failed(self, test_repository, fallback, languages, error, error_msg):
        with pytest.raises(error, match=re.escape(error_msg)):
            test_repository.add_hierarchy(fallback, languages)

    @pytest.mark.parametrize("fallback, language, expected", [
        ("fr", "fr-BE", ['fr-FR']),
        ("fr", "fr-FR", []),
        ("en", "en-US", ["en-GB"]),
        ("se", "se-SE", ["se-FI", "se-NO"]),
    ])
    def test_repository_remove_hierarchy(self, test_repository, fallback, language, expected):
    ...
    @pytest.mark.parametrize("fallback, language, error_msg", [
    ...
    def test_repository_remove_hierarchy_failed(self, test_repository, fallback, language, error_msg):
        with pytest.raises(ValueError, match=re.escape(error_msg)):
            test_repository.remove_hierarchy(fallback, language)
    ...
    def test_repository_update_hierarchy(self, test_repository, fallback, languages, expected):
    ...
    def test_repository_update_hierarchy_failed(self, test_repository, fallback, languages, error, error_msg):
        with pytest.raises(error, match=re.escape(error_msg)):
            test_repository.update_hierarchy(fallback, languages)
